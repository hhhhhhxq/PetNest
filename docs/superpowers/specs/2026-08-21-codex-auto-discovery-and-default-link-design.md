...
用户在 PetNest 中指定的目录优先级最高并作为硬覆盖。进程环境中的 `CODEX_HOME` 是高优先级自动候选，但仍需与 app-server 和默认目录一起验证，避免损坏或过期环境变量永久遮蔽当前真实 profile；最终只监听验证得分最高的一个来源。
...
可能启动 Codex app-server 的 `codexHome` 查询必须在后台线程执行；Qt 主线程只做有界本地路径探测和消费线程结果，打开设置页和 30 秒重试不得阻塞宠物动画或交互。
...
- 没有 `sessions` 时，至少需要存在 `config.toml`、`auth.json`、`.codex-global-state.json` 或 `history.jsonl` 之一作为只读标记；
- 明显不是 Codex 数据目录时不保存，并就地说明需要选择包含 `sessions` 或 Codex 配置标记的目录；
...
- 只有发现旧 PetNest Hook、存在 PetNest 插件安装收据或用户显式开启外部事件服务时才启动本机 Hook 端口；基础 JSONL 监听不依赖该端口。
