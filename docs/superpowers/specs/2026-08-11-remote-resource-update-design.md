...
下载流程按 manifest 中的单个资源（一个光标主题、一个特效或一个倒计时背景）分别创建临时文件集，逐文件核对大小和 SHA-256。一个资源的全部文件验证成功后，复制/链接到新的不可变混合视图，再用临时文件原子替换 `current.json` 指针；因此已完成的资源可以立即生效，后续资源失败不会回滚它。失败资源保留上一代资源，首次安装则继续使用安装包内置默认值。资源内部仍不会出现半套文件。

当当前正在使用光标主题或正在播放某个资源时，不修改其已打开的路径；新版本在下一次启动或用户重新打开对应设置时生效。光标、特效和倒计时背景之间没有业务耦合，允许它们处于不同版本；只要求同一资源包内部保持一致。
