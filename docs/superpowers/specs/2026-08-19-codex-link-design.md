...
- `review`：每次收到任务完成事件都显示完成提示，并保留 PetNest 本地未读标记，直到用户点击/关闭或任务重新开始；Codex 原生是否把当前会话列为未读不影响该提示；
- 多任务显示聚合数量；
- 气泡可点击；点击前通过 Codex 本地只读线程索引把 Hook/续跑 rollout 标识解析成稳定的技术 `thread-id`，再通过公开的 `codex://threads/<thread-id>` 深链在 macOS 或 Windows 上唤起 Codex，并精确打开对应任务；
- 多个未读任务聚合时优先打开最近完成的一条，只清除该任务的本地未读状态，其余未读继续保留；系统未注册协议或拒绝打开时恢复气泡，不丢失未读状态。
