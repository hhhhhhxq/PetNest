...
- 可选的宠物旁轻量便签本；鼠标悬浮宠物后可从道具箱旁打开，支持普通便签、待办清单和提醒事项，内容仅保存在本机。
...
- macOS 系统空闲、会话事件目前是安全 fallback。自动启动位于「设置 → 应用与更新 → 启动管理」：macOS 13+ 打包版使用系统登录项，源码版使用当前用户的 `~/Library/LaunchAgents/com.petnest.source.plist`，下次登录后启动。源码版请保留项目目录和 Python 虚拟环境；关闭开关不会退出正在运行的桌宠，主动退出后不会自动重启。正式发布前仍需完成签名产物的 macOS 回归测试。
...
用量面板的「同账号设备用量排名（预估）」会在同一账号、同一额度周期内按各电脑 Token 总量降序排列。每台设备的预估额度占用按 `设备 Token ÷ 已同步且有匹配记录的设备 Token 合计 × 账号当前已用额度` 动态折算；这只是已知设备之间的比例，不代表 Codex 官方的单机归因，也无法确定未同步设备的占用。排名同时显示每台电脑的 Token、模型请求数、最常用模型，以及实际产生 Token 的模型回合中速度模式和推理强度的占比；旧版日志没有推理强度字段的回合会单列为“未记录”。切换顶部「账号」或「周期」会切换相应排名。
...
局域网包只包含账号哈希、脱敏账号名、套餐、账号已用百分比、设备 ID/显示名、额度周期、分类 Token 计数、模型汇总、速度模式计数、推理强度计数和日志扫描诊断，不包含原始邮箱、提示词、回复、会话文件、文件路径或认证凭据。每次收到的有效快照都会按账号、额度周期和设备 ID 写入本机 `codex-usage-history-devices.json`，重启后仍可读取，旧周期不会被新周期覆盖。该通道复用 UDP `18487`，不提供端到端加密；只应在可信局域网内使用。自动发现的临时设备会在重启后重新发现；手动验证成功的局域网伙伴会保存到 `known-lan-peers.json`，以后启动时自动定向重连。
...
更新或安装会先在临时目录校验，失败时不会留下半成品。当前宠物正在显示下班提醒时，交换中心会暂时锁定该宠物的资源；关闭提醒后即可继续。旧版下班动画包会自动转换成两个必需全屏动作，并可额外提供 `work_finish_lie_loop` 躺下循环；未提供循环素材时仍停在躺下过渡的最后一帧。

分享时直接把页面生成的 ZIP 发给别人即可；接收方不需要知道包内文件名，也不需要手写清单文件。

## 宠物商店

在托盘的「宠物与动作…」窗口中选择「宠物商店」，可以浏览由 PetNest 维护的官方精选完整宠物包。首页提供推荐位、名称／作者／标签搜索和标签筛选；卡片会直接标注「已领养」「可更新」或「本地已有」。进入详情后会按需下载一张轻量 `idle` 预览精灵图，不会为了预览提前下载完整宠物。

- 「领养」会下载完整 ZIP，核对目录声明的大小和 SHA-256，再复用现有宠物包校验、备份和原子安装流程。
- 商店不显示也不要求发布者维护版本号；本机记录成功安装时的 ZIP 摘要，远程摘要变化时显示「可更新」。页面显示的是资源最后更新时间。
- 新领养完成后只刷新宠物库，不会自动切换当前宠物；切换、停用或删除继续由现有宠物库负责。
- 更新当前正在使用的宠物时会原位重新加载；如果运行时拒绝新包，应用会自动回滚安装结果。
- 网络不可用时会继续展示最后一份校验成功的商店目录并标注「离线内容」；首次使用且没有缓存时显示错误和重试入口。

商店目录、封面、预览和 ZIP 存放在私有 `petnest-resources` 仓库，通过 Cloudflare Worker 的独立白名单接口下发。GitHub Token 只存在于 Worker Secret；客户端不保存仓库凭据，也不能读取商品白名单之外的私有文件。
...
- 输入必须是原始透明 PNG 或静态 WebP，支持 `1536 × 1872`（`8×9`）和 `1536 × 2288`（`8×11`），每格均为 `192 × 208` 像素；动画 WebP 会被明确拒绝。
- 输入必须具有 alpha 通道。不要使用聊天截图、拼贴图、缩放后的图片或 JPG。
- 导入默认不覆盖同 ID 的已有宠物包；请改用新 ID，或先手动备份并移除旧包。

这是 Codex `8 × 9` / `8 × 11` 标准图集的行到 PetNest 动作的默认映射；两种格式的前 9 行通用，V2 额外保留两行环视方向。PetNest 会保留已选格位的默认逐帧时长；普通 PNG 序列帧宠物包不限制某一动作只能使用 6 或 8 帧。受固定图集的 8 列限制，单次导入的一行最多选择 8 格；需要更多帧时，可直接向动作目录添加 PNG 序列帧。对标准时长表范围外的手动格位，导入器会按该动作的 FPS 生成默认时长。导入后可直接编辑生成包的 `pet.json` 调整绑定、FPS、优先级或 fallback。

| 图集行 | 原动作 | 导入后的 PetNest 动作 | 说明 |
| --- | --- | --- | --- |
| 0 | idle | `idle` | 默认循环动画。 |
| 1 | running-right | `drag_right` | 向右拖动期间循环播放。 |
| 2 | running-left | `drag_left` | 向左拖动期间循环播放。 |
| 3 | waving | `click` | 单击时播放一次。 |
| 4 | jumping | `hover` | 鼠标悬停时循环播放。 |
| 5 | failed | `error` | 外部错误时播放一次。 |
| 6 | waiting | `waiting` | 等待状态循环。 |
| 7 | running | `working` | 工作状态循环。 |
| 8 | review | `review` | 任务成功后进入待查看状态。 |
| 9 | look directions A | `look_directions` 001–008 | V2 的 000°–157.5° 环视帧。 |
| 10 | look directions B | `look_directions` 009–016 | V2 的 180°–337.5° 环视帧。 |

标准图集没有独立的 `success` 行；生成的包会将 `agent.success` 绑定到 `review`，缺失时安全 fallback 到 `idle`。
...
预警组内部按“开放预警池”工作：每台兼容客户端都持久化一份成员名单，设备建立连接时先比较名单摘要，只交换缺少或版本过期的成员记录。因此名单同步不是第一次连接时的一次性复制。A、D 在两个网段之间建立一次连接后，B 以后经 A 加入、E 以后经 D 加入，这些变化会在 A、D 下次连接时自动补偿传播；桥接一侧暂时离线也不会丢失变化。退出记录会保留版本，长期离线设备携带的旧“已加入”记录不能让成员重新出现。

互动页面分别显示“已加入”“在线”和“可发送”人数。名单中的离线成员继续显示；只有最近在线、完成直接身份与端点验证并且当前可达的成员才会成为聊天和预警收件人。从其他成员转发得到的 IP 只作为待验证线索，验证成功前不会直接发送消息。
...
两个从未建立过任何联系、广播又互相隔离的网段仍然需要一个首次入口：任意一对设备输入一次 IP 即可。桥接建立后，后续成员不需要两两添加；每个已同步客户端都会保存名单和端点，并在成员重新上线后继续校对。该功能不依赖 Firebase、固定目录服务器、写死 IP 或临时协调者。

Windows 安装器会请求管理员权限并创建仅绑定 `PetNest.exe` 的 UDP/TCP `18487` 入站防火墙规则，默认只允许「专用网络」。安装页可选开启「公用网络」；不建议在咖啡店、机场等不可信网络中开启。若之后在公用网络启用局域网互动，而当前程序的规则没有完整放行，Windows 版会在桌宠旁显示不会自动消失的提醒，互动窗口也会保留警告条。点击“一键允许”会触发一次系统管理员确认，并只为当前 `PetNest.exe` 补齐 UDP/TCP `18487` 的专用、公用网络规则；取消后可以重试。macOS 没有 Windows 的专用/公用网络规则分类，因此不显示这项提醒。卸载时会删除 PetNest 创建的规则。若规则创建失败，程序本身仍可正常启动。
...
便签本内容单独保存在同一用户配置目录的 `quick-notebook.json`，不会上传或写入日志。第一版不提供搜索、云同步、团队共享、图片或附件。
