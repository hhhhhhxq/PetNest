...
- 附近设备可通过局域网聊天，支持文字、常用表情和压缩图片；会话记录不落盘。
...
多台电脑可在「Codex 用量」中点击「连接电脑…」，或从托盘「互动…」→「连接电脑 IP」输入对方的局域网 IPv4。定向握手成功后，双方会立即互换各自的本机 Token 快照；连接保持期间每 5 分钟重新同步，在用量面板手动刷新也会触发同步。同步只接受相同账号哈希、相同额度重置时间且来自已握手 IP/端口的数据，并按设备 ID 替换旧快照，不会重复累加。

用量面板的「同账号设备 Token 排名」会在同一账号、同一额度周期内按各电脑 Token 总量降序排列。局域网连接后会互换快照，但排名只纳入账号哈希和额度重置时间都与当前选择匹配的设备。切换顶部「账号」或「周期」会切换到对应账号和当前/历史周期的排名；每个账号至少同步过一次后才会出现其他电脑。Codex 不提供单台电脑的精确额度归因，因此不会把账号整体百分比冒充为单机额度排名。

局域网包只包含账号哈希、设备 ID/显示名、额度周期和分类 Token 计数，不包含邮箱、提示词、回复、会话文件或认证凭据。该通道复用 UDP `18487`，不提供端到端加密；只应在可信局域网内输入对方 IP。手动 IP 和同步信任关系只在当前 PetNest 运行期间有效，重启后需重新连接。
...
托盘菜单中的“局域网互动…”提供附近设备、昵称、快捷互动、短文字、动效和聊天。聊天页按当前选中设备分开会话，可发送最多 2,000 字的文字、常用表情和图片。原图不会被修改；发送前会缩放至最长边 1,600 像素并转为不超过 1.5 MB 的 JPEG。聊天记录只保留在当前 PetNest 进程内，关闭后不保存。

应用默认开启局域网发现，也可在设置或互动页面关闭“允许附近设备发现我”。昵称保存在用户设置中；未设置时使用 `用户-短设备码`。快捷互动消息只携带类型、目标设备 ID、发送方名称和文字或动效 ID，不传输宠物图片、Lottie JSON 或 PNG 资源。动效接收端按本地同 ID 的 `effect.json` 播放，因此双方需要预先安装同名动效包。

发现、快捷互动和 Codex 用量同步使用 UDP，聊天使用有长度分帧的 TCP，两者都使用固定数字端口 `18487`。如果两个设备处于不同网段、自动广播发现不到，可在互动窗口点击“连接电脑 IP”输入对方 IPv4。定向握手成功后才会把设备加入列表，并每 8 秒刷新一次；连续 24 秒没有回应才移除。手动地址只在本次运行中使用，不会保存。两个网段之间必须允许路由互通，且双方防火墙需同时放行 UDP 和 TCP `18487`。接收端会核对已握手的设备 ID 与 IP，并限制消息大小、内容类型和单个设备的发送频率。局域网聊天没有端到端加密，不要在不可信网络中发送密码、令牌或其他敏感内容。
