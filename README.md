...
在系统托盘中选择「编辑动画时长…」。列表会说明每个动作在什么时机展示、帧数和当前总时长。选择一个动作后，直接输入该动作想要的总时长（毫秒）；数值越小，播放越快，同时会保留原有的帧间节奏。需要精细调整某几帧的停顿时，点击当前动作专属的「编辑逐帧节奏（高级）」；切换到其他动作时该面板会自动收起。设置仅保存在当前计算机的用户配置中，**不会修改** `pet.json` 或 PNG 资源；保存后立刻重载当前宠物。
