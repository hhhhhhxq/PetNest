...
- 附近设备可私聊或进入局域网群聊，支持文字、常用表情和压缩图片；会话记录不落盘。
...
用量面板的「同账号设备用量排名（预估）」会在同一账号、同一额度周期内按各电脑 Token 总量降序排列。每台设备的预估额度占用按 `设备 Token ÷ 已同步设备 Token 合计 × 账号当前已用额度` 动态重算，因此所有已同步设备的估算合计等于账号已用百分比；未同步设备、模型及输入/缓存/输出权重差异都会带来偏差，界面会始终标明“预估”。排名同时显示每台电脑的 Token、模型请求数、最常用模型，以及实际产生 Token 的模型回合中“极快”（`priority`）和“标准”（`default`）的占比。切换顶部「账号」或「周期」会切换相应排名。

局域网包只包含账号哈希、脱敏账号名、套餐、账号已用百分比、设备 ID/显示名、额度周期、分类 Token 计数、模型汇总和速度模式计数，不包含原始邮箱、提示词、回复、会话文件或认证凭据。每次收到的有效快照都会按账号、额度周期和设备 ID 写入本机 `codex-usage-history-devices.json`，重启后仍可读取，旧周期不会被新周期覆盖。该通道复用 UDP `18487`，不提供端到端加密；只应在可信局域网内使用。设备在线关系会在重启后重新发现，手动输入的 IP 不保存。
