...
- 系统托盘提供显示/隐藏、暂停、切换宠物、导入精灵图、逐动作速度编辑、重新加载、设置和退出。
...
在系统托盘中选择「编辑动画速度…」。列表会说明每个动作在什么时机展示、帧数、总时长和速度。选择一个动作后可调节速度倍率（例如 `1.50×` 更快），需要时勾选「逐帧时长（高级）」逐帧编辑毫秒数。设置仅保存在当前计算机的用户配置中，**不会修改** `pet.json` 或 PNG 资源；保存后立刻重载当前宠物。
