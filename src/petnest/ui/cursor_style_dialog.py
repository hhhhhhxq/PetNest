...
class CursorStyleDialog(QDialog):
    def __init__(self, settings: Settings, cursor_styles: list[CursorStyle], parent: QDialog | None = None) -> None:
        ...
        restore = QPushButton("恢复 Windows 默认样式", self)
    ...
    def _update(self) -> None:
        ...
        self.cursor_preview.setText("使用 Windows 系统默认样式" if style is None else "")
        ...
        for role, label in (("arrow", "普通箭头"), ("busy", "忙碌中"), ("text", "文本选择"), ("move", "拖拽/移动"), ("resize_horizontal", "水平调整"), ("resize_vertical", "垂直调整"), ("resize_diag_1", "左上↘右下调整"), ("resize_diag_2", "右上↙左下调整")):
            self._role_layout.addRow(label, QLabel("主题样式" if style and role in style.roles else "使用系统默认", self))
